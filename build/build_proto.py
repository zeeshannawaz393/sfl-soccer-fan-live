...
/* Dark-mode: active segmented-tab must stand out from the track (Email / Phone OTP etc.) */
body[data-stage="dark"] #mount>.phone .seg i.on,
body[data-stage="dark"] [id^="j"]:not(.sflgiftwrap)>.phone:not(.splash):not(.welcome):not(.room) .seg i.on{
  background:#2E3947!important;color:#F4F6FA!important;box-shadow:0 2px 8px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,255,255,.10)!important;
}
body[data-stage="dark"] #mount>.phone,
body[data-stage="dark"] [id^="j"]:not(.sflgiftwrap)>.phone:not(.splash):not(.welcome):not(.room) .inp,
body[data-stage="dark"] #mount>.phone,
...
/* Manager HQ hero is light in light mode — status bar text must be dark to stay visible */
body[data-stage="light"] #j13 .sfl-statusbar{color:#14161C!important}
...
body[data-stage="light"] #j13 .recruit .rh span{color:#0A7A38!important}
body[data-stage="light"] #j13 .note.volt{background:rgba(15,183,83,.10)!important;border-color:rgba(15,183,83,.30)!important;color:#2E7D14!important}
...
body[data-stage="light"] #j13 .rewcard.claim .stbadge.c{
  background:#fff!important;color:#0A7A38!important;border-color:rgba(15,183,83,.45)!important;
}
...
%SB%
  ...
  function sflToast(msg){
    ...
    if(!document.getElementById('sfltoast-css')){ var _ts=document.createElement('style'); _ts.id='sfltoast-css'; _ts.textContent='@keyframes sfltoastin{0%{opacity:0;transform:translateY(-135%)}70%{opacity:1;transform:translateY(4%)}100%{opacity:1;transform:translateY(0)}}@keyframes sfltoastout{to{opacity:0;transform:translateY(-135%)}}'; document.head.appendChild(_ts); }
    msg=String(msg);
    var sp=msg.indexOf(' '); var first=sp>0?msg.slice(0,sp):'';
    var hasIcon=!!first && !/[a-zA-Z0-9]/.test(first) && first.length<=4;
    var icon=hasIcon?first:''; var text=hasIcon?msg.slice(sp+1):msg;
    var okic=(icon==='✓'||icon==='✔'||icon==='✅'); var warnic=(icon==='✕'||icon==='✗'||icon==='⚠'||icon==='⚠️'||icon==='🔒');
    var tone=okic||/added|sent|saved|registered|removed|claimed|copied|shared|confirmed|ready|unlocked|accepted|updated|deducted|secured|following/i.test(text)?'ok':(warnic||/not enough|need |failed|error|declined|first|only |offline|pending/i.test(text)?'warn':'info');
    var accent=tone==='ok'?'#7BE28A':(tone==='warn'?'#FFC24D':'#7BB8FF');
    var iconBg=tone==='ok'?'linear-gradient(150deg,#38E08A,#0FA855)':(tone==='warn'?'linear-gradient(150deg,#FFC24D,#FF9E1B)':'linear-gradient(150deg,#5AA9FF,#2266C9)');
    var glow=tone==='ok'?'rgba(56,224,138,.4)':(tone==='warn'?'rgba(255,158,27,.4)':'rgba(47,127,209,.4)');
    var title=tone==='ok'?'Success':(tone==='warn'?'Heads up':'Notification');
    var iconCol='#fff';
    if(!icon) icon=tone==='ok'?'✓':(tone==='warn'?'!':'🔔');
    var tst=document.createElement('div'); tst.className='sfltoast';
    tst.style.cssText='position:absolute;top:56px;left:14px;right:14px;z-index:900;display:flex;align-items:center;gap:12px;cursor:pointer;background:linear-gradient(180deg,rgba(34,39,52,.98),rgba(20,24,33,.98));font-family:Manrope,-apple-system,sans-serif;padding:12px 14px;border-radius:18px;border:1px solid rgba(255,255,255,.13);box-shadow:0 22px 50px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.06);-webkit-backdrop-filter:blur(16px);backdrop-filter:blur(16px);animation:sfltoastin .34s cubic-bezier(.2,.9,.3,1.15)';
    tst.innerHTML='<span style="flex:none;width:38px;height:38px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:800;color:'+iconCol+';background:'+iconBg+';box-shadow:0 6px 16px '+glow+'">'+icon+'</span>'
      +'<div style="flex:1;min-width:0;font-size:13px;font-weight:750;color:#F4F6FA;line-height:1.35">'+text+'</div>';
    phone.appendChild(tst);
    function _kill(){ if(!tst.parentNode)return; if(tst._x)return; tst._x=1; tst.style.animation='sfltoastout .28s ease forwards'; setTimeout(function(){ if(tst.parentNode)tst.parentNode.removeChild(tst); },280); }
    tst.addEventListener('click',function(e){ e.stopPropagation(); if(tst._t)clearTimeout(tst._t); _kill(); });
    tst._t=setTimeout(_kill,1900);
  }
  var SFLclubBattle=null;
  function applyClubBattle(root){
    if(!SFLclubBattle){ return; }
    var o=SFLclubBattle; var oppShort=(o[0]||'Rival').split(' ')[0];
    var lb=root.querySelector('.livetop .lbadge'); if(lb)lb.innerHTML='<i></i>CLUB BATTLE';
    function crest(el,txt,col){ if(!el)return; el.textContent=txt; if(col){el.style.background='linear-gradient(150deg,rgba(255,255,255,.25),'+col+' 62%)';} }
    crest(root.querySelector('.pkc:not(.r) .crest'),o[3]||'BW',o[2]); crest(root.querySelector('.pkc.r .crest'),'RF','#E4362B');
    var ln=root.querySelector('.pkc:not(.r) .nm'); if(ln)ln.textContent=o[0]+' · '+o[1];
    var rn=root.querySelector('.pkc.r .nm'); if(rn)rn.textContent='Red Fury · You';
    crest(root.querySelector('.fpside:not(.r) .crest'),o[3]||'BW',o[2]); crest(root.querySelector('.fpside.r .crest'),'RF','#E4362B');
    var lfs=root.querySelector('.fpside:not(.r) .n'); if(lfs)lfs.textContent=oppShort;
    var rfs=root.querySelector('.fpside.r .n'); if(rfs)rfs.textContent='Red Fury';
    var gb=root.querySelector('.giftbtns .gbtn.blue'); if(gb)gb.textContent='🎁 Gift '+o[0];
    var gr=root.querySelector('.giftbtns .gbtn.red'); if(gr)gr.textContent='🎁 Gift Red Fury';
  }
  function showSupportNew(cat){
    var phone=_qaPhone(); if(!phone)return; var old=phone.querySelector('.sflsup'); if(old)old.remove();
    var cats=['Payments & Coins','Withdrawal','Escrow / Transfer','Fraud / Abuse','Other'];
    var _supcat=cat||'Payments & Coins';
    var w=document.createElement('div'); w.className='sflsup';
    ...
    function chip(c){ var on=c===_supcat; return '<div class="sup-cat" data-c="'+c+'" style="padding:8px 12px;border-radius:999px;font-size:12px;font-weight:800;cursor:pointer;border:1px solid '+(on?'#4AA0F5':'rgba(255,255,255,.16)')+';background:'+(on?'rgba(74,160,245,.18)':'rgba(255,255,255,.06)')+';color:'+(on?'#8CC6FF':'#C9D0DC')+'">'+c+'</div>'; }
    w.innerHTML='<div style="width:100%;background:linear-gradient(180deg,#1b2233,#12151d);border-top-left-radius:22px;border-top-right-radius:22px;padding:16px 18px 24px;box-shadow:0 -20px 50px rgba(0,0,0,.5)">'
      +'<div style="width:38px;height:4px;border-radius:2px;background:rgba(255,255,255,.2);margin:0 auto 14px"></div>'
      +'<div style="font-size:16px;font-weight:800;color:#fff">New support ticket</div>'
      +'<div style="font-size:12px;font-weight:650;color:#8892A4;margin-top:3px">Pick a topic and describe the issue</div>'
      +'<div style="display:flex;flex-wrap:wrap;gap:7px;margin-top:14px">'+cats.map(chip).join('')+'</div>'
      +'<div class="sup-in qa-ce" contenteditable="true" data-ph="Describe your issue…" style="min-height:64px;margin-top:12px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.14);border-radius:13px;padding:11px 13px;font-size:13.5px;font-weight:600;color:#fff;outline:none"></div>'
      +'<div style="display:flex;gap:9px;margin-top:14px"><div class="sup-cancel" style="flex:1;text-align:center;padding:12px;border-radius:12px;background:rgba(255,255,255,.08);color:#C9D0DC;font-weight:800;font-size:13.5px;cursor:pointer">Cancel</div><div class="sup-send" style="flex:1.6;text-align:center;padding:12px;border-radius:12px;background:linear-gradient(140deg,#5AA9FF,#2266C9);color:#fff;font-weight:800;font-size:13.5px;cursor:pointer">Create ticket</div></div></div>';
    phone.appendChild(w);
    setTimeout(function(){try{w.querySelector('.sup-in').focus();}catch(e){}},60);
    w.addEventListener('click',function(e){ e.stopPropagation();
      var c=e.target.closest('.sup-cat'); if(c){ _supcat=c.getAttribute('data-c'); [].forEach.call(w.querySelectorAll('.sup-cat'),function(x){var on=x===c; x.style.borderColor=on?'#4AA0F5':'rgba(255,255,255,.16)'; x.style.background=on?'rgba(74,160,245,.18)':'rgba(255,255,255,.06)'; x.style.color=on?'#8CC6FF':'#C9D0DC';}); return; }
      if(e.target===w||e.target.closest('.sup-cancel')){ w.remove(); return; }
      if(e.target.closest('.sup-send')){ w.remove(); goTo('ticketdetail'); setTimeout(function(){ sflToast('✓ Ticket created · '+_supcat+' — support will reply shortly'); },40); return; } });
  }
  function showClubBattle(presetIdx){
    var phone=_qaPhone(); if(!phone)return; var old=phone.querySelector('.sflbattle'); if(old)old.remove();
    var OPP=[['Blue Wolves','M. Okonkwo','#2266C9','BW','3–2'],['North Stars','S. Adeyemi','#E6A100','NS','2–1'],['Violet Valley','K. Rivera','#7C3AED','VV','4–2'],['Greenfield','T. Bennett','#0FA855','GF','5–0']];
    var w=document.createElement('div'); w.className='sflbattle';
    w.style.cssText='position:absolute;inset:0;z-index:84;display:flex;align-items:flex-end;background:rgba(4,6,10,.6);-webkit-backdrop-filter:blur(3px);backdrop-filter:blur(3px);font-family:Manrope,-apple-system,sans-serif';
    var card=document.createElement('div'); card.className='sflbattle-card';
    card.style.cssText='width:100%;background:linear-gradient(180deg,#1b2233,#12151d);border-top-left-radius:22px;border-top-right-radius:22px;padding:16px 18px 26px;box-shadow:0 -20px 50px rgba(0,0,0,.55)';
    w.appendChild(card); phone.appendChild(w);
    function grab(){ return '<div style="width:38px;height:4px;border-radius:2px;background:rgba(255,255,255,.2);margin:0 auto 14px"></div>'; }
    function pick(){
      card.innerHTML=grab()
        +'<div style="font-size:17px;font-weight:800;color:#fff">⚔️ Club Battle</div>'
        +'<div style="font-size:12px;font-weight:650;color:#8892A4;margin-top:3px">Challenge a rival club\\'s manager to a live PK battle — winner decided by confirmed support.</div>'
        +'<div style="margin-top:14px;display:flex;flex-direction:column;gap:9px">'
        + OPP.map(function(o,i){ return '<div class="bt-opp" data-i="'+i+'" style="display:flex;align-items:center;gap:11px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:10px 12px;cursor:pointer"><div style="width:36px;height:36px;flex:none;border-radius:11px;display:flex;align-items:center;justify-content:center;font-size:10.5px;font-weight:800;color:#fff;background:linear-gradient(150deg,rgba(255,255,255,.28),'+o[2]+' 62%);box-shadow:inset 0 1px 0 rgba(255,255,255,.3),0 3px 8px rgba(0,0,0,.3)">'+o[3]+'</div><div style="flex:1;min-width:0"><div style="font-size:13.5px;font-weight:800;color:#fff">'+o[0]+'</div><div style="font-size:11px;font-weight:650;color:#8892A4">Manager · '+o[1]+'</div></div><span style="font-size:12px;font-weight:800;color:#C9FF3D;background:rgba(201,255,61,.14);border:1px solid rgba(201,255,61,.3);padding:6px 12px;border-radius:999px">Challenge</span></div>'; }).join('')
        +'</div>';
    }
    function sent(o){
      card.innerHTML=grab()+'<div style="text-align:center;padding:8px 0 4px"><div style="font-size:34px">⚔️</div><div style="font-size:16px;font-weight:800;color:#fff;margin-top:8px">Challenge accepted!</div><div style="font-size:12px;font-weight:650;color:#8892A4;margin-top:4px">'+o[1]+' · '+o[0]+' is ready — starting the battle…</div></div>';
      setTimeout(function(){ if(w.parentNode){ w.remove(); SFLpkViewer=false; SFLclubBattle=o; goTo('pkbattle'); } }, 1400);
    }
    function result(o){
      card.innerHTML=grab()+'<div style="text-align:center"><div style="font-size:34px">🏆</div>'
        +'<div style="font-size:19px;font-weight:800;color:#C9FF3D;margin-top:6px">Red Fury won!</div>'
        +'<div style="font-size:13px;font-weight:700;color:#EAEEF5;margin-top:4px">Red Fury '+o[4]+' '+o[0]+'</div>'
        +'<div style="display:inline-flex;align-items:center;gap:7px;margin-top:14px;background:rgba(255,194,61,.15);border:1px solid rgba(255,194,61,.4);color:#FFC24D;font-weight:800;font-size:15px;padding:9px 16px;border-radius:999px"><div class="coin" style="width:20px;height:20px;font-size:10px">C</div>+10 Coins</div></div>'
        +'<div class="bt-collect" style="text-align:center;margin-top:18px;padding:13px;border-radius:13px;background:linear-gradient(140deg,#C9FF3D,#8FD400);color:#10140A;font-weight:800;font-size:14px;cursor:pointer">Collect reward</div>';
    }
    if(typeof presetIdx==='number'&&OPP[presetIdx]){ sent(OPP[presetIdx]); } else { pick(); }
    w.addEventListener('click',function(e){ e.stopPropagation();
      var op=e.target.closest('.bt-opp'); if(op){ sent(OPP[+op.getAttribute('data-i')]); return; }
      if(e.target.closest('.bt-collect')){ w.remove(); sflToast('✓ Club Battle won · +10 Coins added to the club'); return; }
      if(e.target===w){ w.remove(); return; } });
  }
  function showRematchSim(){
    var phone=_qaPhone(); if(!phone)return; var old=phone.querySelector('.sflrematch'); if(old)old.remove();
    var oppName=SFLclubBattle?(SFLclubBattle[1]+' · '+SFLclubBattle[0]):'Maria · Blue Wolves';
    var w=document.createElement('div'); w.className='sflrematch';
    w.style.cssText='position:absolute;inset:0;z-index:94;display:flex;align-items:center;justify-content:center;background:rgba(4,6,10,.82);-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px);font-family:Manrope,-apple-system,sans-serif';
    w.innerHTML='<div style="width:82%;max-width:300px;text-align:center;background:linear-gradient(180deg,#1b2233,#12151d);border:1px solid rgba(255,255,255,.14);border-radius:22px;padding:24px 20px;box-shadow:0 30px 70px rgba(0,0,0,.6)"><div style="font-size:34px">⚔️</div><div style="font-size:18px;font-weight:800;color:#fff;margin-top:8px">Rematch requested</div><div class="rm-sub" style="font-size:12.5px;font-weight:650;color:#8892A4;margin-top:6px">Waiting for '+oppName+'…</div></div>';
    phone.appendChild(w);
    setTimeout(function(){ var s=w.querySelector('.rm-sub'); if(s)s.innerHTML='<b style="color:#C9FF3D">'+oppName+' accepted!</b> Restarting the battle…'; },1000);
    setTimeout(function(){ if(w.parentNode){ w.remove(); goTo('pkbattle'); } },2100);
  }
  ...
  function showLeaveConfirm(){
    ...
    phone.appendChild(w);
    w.addEventListener('click',function(e){ e.stopPropagation(); if(e.target===w||e.target.closest('.lv-stay')){ w.remove(); return; } if(e.target.closest('.lv-go')){ w.remove(); SFLpip=null; showViewerRecap(); return; } });
  }
  function showViewerRecap(){
    var phone=document.getElementById('scaler').firstElementChild.querySelector('.phone'); if(!phone)return; var old=phone.querySelector('.sflrecap'); if(old)old.remove();
    var w=document.createElement('div'); w.className='sflrecap';
    w.style.cssText='position:absolute;inset:0;z-index:92;display:flex;align-items:center;justify-content:center;background:rgba(4,6,10,.82);-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px);font-family:Manrope,-apple-system,sans-serif';
    function stat(ic,v,l){ return '<div style="flex:1;text-align:center;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:12px 6px"><div style="font-size:17px">'+ic+'</div><div style="font-size:18px;font-weight:800;color:#fff;margin-top:4px">'+v+'</div><div style="font-size:9.5px;font-weight:700;color:#8892A4;margin-top:2px">'+l+'</div></div>'; }
    w.innerHTML='<div style="width:84%;max-width:312px;background:linear-gradient(180deg,#1b2233,#12151d);border:1px solid rgba(255,255,255,.14);border-radius:22px;padding:22px 20px;text-align:center;box-shadow:0 30px 70px rgba(0,0,0,.6)">'
      +'<div style="font-size:32px">👋</div>'
      +'<div style="font-size:19px;font-weight:800;color:#fff;margin-top:6px">You left the room</div>'
      +'<div style="font-size:12.5px;font-weight:650;color:#8892A4;margin-top:4px">Matchday Warm-Up · RobbieOnAir</div>'
      +'<div style="display:flex;gap:9px;margin-top:16px">'+stat('⏱','24m','Watched')+stat('💬','8','Messages')+stat('🎁','2','Gifts sent')+'</div>'
      +'<div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:12px;font-size:11px;font-weight:750;color:#C9FF3D;background:rgba(201,255,61,.1);border:1px solid rgba(201,255,61,.25);border-radius:999px;padding:7px 12px">🔥 You were part of a 2.1k crowd</div>'
      +'<div class="rc-done" style="margin-top:18px;padding:13px;border-radius:13px;background:linear-gradient(140deg,#C9FF3D,#8FD400);color:#10140A;font-weight:800;font-size:14px;cursor:pointer">Back to Stadium</div></div>';
    phone.appendChild(w);
    w.addEventListener('click',function(e){ e.stopPropagation(); if(e.target.closest('.rc-done')||e.target===w){ w.remove(); goTo('live'); return; } });
  }
  ...
  function render(){
    ...
    if(scr.fnum==='FT-01'){ var _mbt=mount.querySelector('.mgr-battle'); if(_mbt)_mbt.style.display=SFLmgrMode?'':'none'; }
    ...
    if(scr.fnum==='PK-00'){SFLpkViewer=false;SFLclubBattle=null;}
    if(scr.fnum==='PK-03'){applyClubBattle(mount);}
    ...
    if((scr.fnum==='PK-04A'||scr.fnum==='PK-04C')&&SFLpkViewer&&!SFLclubBattle){var _reb=mount.querySelector('.winbtns .re'); if(_reb)_reb.style.display='none'; var _exb=mount.querySelector('.winbtns .ex'); if(_exb){_exb.style.flex='1';_exb.textContent='Exit to Stadium';}}
    if((scr.fnum==='PK-04A'||scr.fnum==='PK-04C')&&SFLclubBattle){var _ceb=mount.querySelector('.winbtns .ex'); if(_ceb)_ceb.textContent='Back to Tasks';}
    ...
    if(scr.fnum==='G-03'){applyPlNotifs(mount);applyCmgrNotifs(mount);applyTourn(mount); var _bn=mount.querySelector('.mgr-battle-notif'); if(_bn)_bn.style.display=SFLmgrMode?'':'none';}
  ...
  function showAskCompose(nm){
    ...
    function toQA(q){
      ...
      w._min=minimize;
      function ans(a){ if(w._iv)clearInterval(w._iv); restore(); w._qa=false; w._answered=true; if(typeof pushComment==='function') pushComment('💬 <b style="color:#FFE1A0">'+nm+'</b> answered: '+a,'askmsg');
    ...
    w.addEventListener('click',function(e){ e.stopPropagation();
      ...
      if(w._answered && !e.target.closest('.sflask-card')){ w.remove(); return; }
  ...
  function showShareSheet(pos){
    ...
    var isLive=!!pos;
    var title=isLive?('Share invite link · '+pos):'Share recruitment invite';
    var link=isLive?('sfl.live/join/redfury-'+String(pos).toLowerCase()):'sfl.app/join/red-fury-12345';
    var doneLabel=isLive?'Done · continue live':'Done';
    var doneToast=isLive?'✓ Invite shared — back to your live':'✓ Recruitment invite shared · fans apply to join';
    var w=document.createElement('div'); w.className='sflshare';
    w.style.cssText='position:absolute;inset:0;z-index:82;display:flex;align-items:flex-end;background:rgba(4,6,10,.55);-webkit-backdrop-filter:blur(2px);backdrop-filter:blur(2px);font-family:Manrope,-apple-system,sans-serif';
    function app(k,ic,c){ return '<div class="sh-app" data-app="'+k+'" style="flex:1;text-align:center;cursor:pointer"><div style="width:50px;height:50px;margin:0 auto;border-radius:15px;display:flex;align-items:center;justify-content:center;font-size:23px;font-weight:800;color:#fff;background:'+c+'">'+ic+'</div><div style="font-size:9.5px;font-weight:750;color:#C9D0DC;margin-top:6px">'+k+'</div></div>'; }
    w.innerHTML='<div style="width:100%;background:linear-gradient(180deg,#1b2233,#12151d);border-top-left-radius:22px;border-top-right-radius:22px;padding:16px 16px 26px;box-shadow:0 -20px 50px rgba(0,0,0,.5)">'
      +'<div style="width:38px;height:4px;border-radius:2px;background:rgba(255,255,255,.2);margin:0 auto 14px"></div>'
      +'<div style="font-size:16px;font-weight:800;color:#fff">'+title+'</div>'
      +'<div style="font-size:12px;font-weight:650;color:#8892A4;margin-top:3px">'+link+'</div>'
      +'<div style="display:flex;gap:6px;margin-top:16px">'+app('WhatsApp','💬','#1FAF54')+app('Facebook','f','#1877F2')+app('TikTok','♪','#000')+app('Instagram','📸','linear-gradient(140deg,#F58529,#DD2A7B,#8134AF)')+app('Copy','🔗','rgba(255,255,255,.12)')+'</div>'
      +'<div class="sh-done" style="text-align:center;margin-top:18px;padding:12px;border-radius:12px;background:linear-gradient(140deg,#C9FF3D,#8FD400);color:#10140A;font-weight:800;font-size:14px;cursor:pointer">'+doneLabel+'</div></div>';
    phone.appendChild(w);
    w.addEventListener('click',function(e){ e.stopPropagation();
      if(e.target===w||e.target.closest('.sh-done')){ w.remove(); sflToast(doneToast); return; }
      var a=e.target.closest('.sh-app'); if(a){ var k=a.getAttribute('data-app'); w.remove(); sflToast(k==='Copy'?'🔗 Link copied':('✓ Shared via '+k)); return; } });
  }
  ...
  function applyCmgrPick(root){
    ...
    [].forEach.call(root.querySelectorAll('.fanrow'),function(r){
      ...
      if(SFLremovedFans[fid]){ r.style.display='none'; return; } else { r.style.display=''; }
    ...
    var _ftitle=root.querySelector('.htitle'); if(_ftitle&&/Squad · \\d+ Fans/.test(_ftitle.textContent||'')){ _ftitle.textContent='Squad · '+(388-Object.keys(SFLremovedFans).length)+' Fans'; }
  }
  ...
  function destOf(t){
    ...
    var _hf=t.closest('.hfol'); if(_hf){ if(_hf.classList.contains('add'))return 'followlist'; return _hf.querySelector('.flv')?'liveroom':'userprofile'; }
  ...
  stage.addEventListener('click',function(e){
    ...
    if(_cf==='CC-01T'){ var _ftr=t.closest('.teamrow'); if(_ftr&&_ftr.getAttribute('data-full')){ var _ffn=((_ftr.querySelector('.tn')||{}).textContent||'This club').trim(); sflToast('⚠ '+_ffn+' already has 5 Fan Clubs — pick another club'); return; } }
    ...
    if(_cf==='G-03'){ if(t.closest('.top .back')){goBack();return;} if(t.closest('.ico')){[].forEach.call(mEl.querySelectorAll('.unread'),function(u){u.style.display='none';}); sflToast('All notifications marked read'); return;} var _nfc=t.closest('.fchip'); if(_nfc){singleSel(_nfc,_nfc.parentElement);return;}
      var _ngo=t.closest('.nrow[data-goto]'); if(_ngo){ var _un2=_ngo.querySelector('.unread'); if(_un2)_un2.style.display='none'; goTo(_ngo.getAttribute('data-goto')); return; }
      var _btl=t.closest('[data-btldec]'); if(_btl){ var _bd=_btl.getAttribute('data-btldec'); var _bnr=_btl.closest('.nrow'); if(_bd==='accept'){ if(_bnr)_bnr.style.display='none'; showClubBattle(0); } else { if(_bnr){ var _bun=_bnr.querySelector('.unread'); if(_bun)_bun.style.display='none'; var _bna=_bnr.querySelector('.nacts'); if(_bna)_bna.innerHTML='<span style="font-size:11px;font-weight:800;color:var(--t3)">Declined</span>'; } sflToast('Club Battle declined'); } return; }
    ...
    if(t.closest('.pkcard')){SFLpkViewer=true;SFLclubBattle=null;goTo('pkbattle');return;}
    ...
    if(_cf.indexOf('CS-')===0 && ((VIEWS[curJ].screens[curS]||{}).srcJ===25)){
      ...
      if(_csbk){ if(/🔔/.test(_csbk.textContent||'')){goTo('csnotes');return;} if(/🧾/.test(_csbk.textContent||'')){goTo('cshistory');return;} if(_cf==='CS-30'){cleanTo('profile', /^CS-/);return;} goBack(); return; }
    ...
    if(_cf==='G-07A'){ if(t.closest('.top .back')){goBack();return;} if(t.closest('.ticket')){goTo('ticketdetail');return;} var _sc=t.closest('.cat'); if(_sc){ showSupportNew(((_sc.querySelector('.ct')||{}).textContent||'').trim()); return;} if(t.closest('.altlink')){goTo('security');return;} if(t.closest('.btn')){showSupportNew('');return;} var _ssr=t.closest('.card'); if(_ssr&&/search help/i.test(_ssr.textContent||'')){showSupportNew('');return;} return; }
    if(_cf==='G-07D'){ if(t.closest('.top .back')){goBack();return;} if(t.closest('.btn')){sflToast('✓ Reply sent to support');return;} return; }
    ...
    if(_cf.indexOf('PK-')===0){
      ...
      if(_cf==='PK-04A'||_cf==='PK-04C'){ var _wb=t.closest('.b'); if(_wb){ if((_wb.classList.contains('re')||/rematch/i.test(_wb.textContent))&&!SFLpkViewer){ if(window._sflpkto)clearTimeout(window._sflpkto); if(window._sflpkfin){clearInterval(window._sflpkfin);window._sflpkfin=null;} showRematchSim(); return;} if(window._sflpkto)clearTimeout(window._sflpkto); if(window._sflpkfin){clearInterval(window._sflpkfin);window._sflpkfin=null;} if(SFLclubBattle){ SFLclubBattle=null; cleanTo('tasks', /^PK-/); return; } goTo(SFLpkViewer?'live':'liveroomhost');return;} return; }
    ...
    if(_cf.indexOf('FT-')===0){
      ...
      if(_cf==='FT-01'||_cf==='FT-02'){
        ...
        var _row=t.closest('.trow'); if(_row){ var _tt=((_row.querySelector('.tt')||{}).textContent||'').toLowerCase();
          if(/club battle/.test(_tt)){showClubBattle();return;}
    ...
    if(_cf.indexOf('PR-')===0||_cf.indexOf('TR-')===0){
      var _prbk=t.closest('.top .back'); if(_prbk){ if(_cf==='TR-06'){cleanTo('prohub', /^TR-0/);return;} if(_cf==='TR-07'){cleanTo('tourndash', /^TR-0[78]/);return;} goBack();return;}
      if(_cf.indexOf('TR-')===0){
        ...
        if(_cf==='TR-06'){ if(t.closest('.top .back')){cleanTo('prohub', /^TR-0/);return;} if(t.closest('.btn')){goTo('tournpayout');return;} if(t.closest('.ruleslink')){goTo('tournrules');return;} if(t.closest('.altlink')){goTo('tournteam');return;} return; }
        if(_cf==='TR-07'){ if(t.closest('.top .back')){cleanTo('tourndash', /^TR-07/);return;} if(t.closest('.btn')){cleanTo('tourndash', /^TR-07/);return;} if(t.closest('.altlink')){goTo('tournteam');return;} return; }
    ...
    if(_cf==='EV-01'){ if(t.closest('.back')){goBack();return;} var _ev=t.closest('.evcard'); if(_ev&&t.closest('.evjoin')){var _et=(_ev.textContent||'').toLowerCase(); if(/pk|tournament/.test(_et)){SFLpkViewer=true;SFLclubBattle=null;goTo('pkbattle');return;} if(/predict|league/.test(_et)){goTo('predictions');return;} if(/recruit/.test(_et)){goTo('mgrrecruit');return;} goTo('live');return;} return; }
    if(_cf==='J2-16'){
      ...
      var _lv16=t.closest('.lvcard'); if(_lv16){var _lvb2=_lv16.querySelector('.lvb'); if(_lvb2&&/pk/i.test(_lvb2.textContent)){SFLpkViewer=true;SFLclubBattle=null;goTo('pkbattle');return;} goTo('liveroom');return;}
      if(t.closest('.btn.club')){goTo('liveroom');return;}
      return;
    }
    ...
    if(_cf.indexOf('J2-')===0){
      ...
      if(_cf==='J2-01'){ if(t.closest('.top .back')){goTo('clubs');return;} var _b1=t.closest('.btn'); if(_b1){ if(/recruit/i.test(_b1.textContent)){goTo('clubdetail');return;} goTo('clubs');return;} return; }
    ...
    if(_cf==='MC-06R'){ if(t.closest('.htop .back')){cleanTo('mgrrecruit', /^MC-06/);return;} var _r6=t.closest('.btn,.altlink'); if(_r6){ if(/another/i.test(_r6.textContent)){goTo('mgraddid');return;} goTo('mgrapplications');return;} return; }
    ...
    if(_cf==='MC-04B'){ if(t.closest('.htop .back')){goBack();return;} var _ro=t.closest('.reasonopt'); if(_ro){singleSel(_ro,_ro.parentElement);return;} var _b4b=t.closest('.btn,.altlink'); if(_b4b){ if(/confirm/i.test(_b4b.textContent)){ var _rn=(SFLcmgrPick&&SFLcmgrPick.name)||'Fan'; if(SFLcmgrPick&&SFLcmgrPick.id)SFLremovedFans[SFLcmgrPick.id]=true; cleanTo('mgrfanlist', /^MC-04/); setTimeout(function(){ sflToast('✓ '+_rn+' removed from the club'); },40); return; } goBack();return;} return; }
    if(_cf.indexOf('ML-')===0){
      if(t.closest('.top .back')){goBack();return;}
    ...
    var hf=t.closest('.hfol'); if(hf){ if(/(^|\s)add(\s|$)/.test(hf.className)){goTo('followlist');} else if(hf.querySelector('.flv')){goTo('liveroom');} else {goTo('userprofile');} return;}
    ...
    var _lvc=t.closest('.livecard,.lvcard'); if(_lvc){ var _lvb=_lvc.querySelector('.lb'); if(_lvb&&/pk/i.test(_lvb.textContent)){SFLpkViewer=true;SFLclubBattle=null;goTo('pkbattle');return;} goTo('liveroom');return; }
    ...
    if(/preview club feed/i.test(lbl)||/enter squad|squad room/i.test(lbl)){goTo(/squad/i.test(lbl)?'liveroom':'club');return;}
    ...
    if(t.closest('.searchbar'))return;
    if(t.matches&&t.matches('.phone,.body,.dbody,.lbody,.scroll,.scrollarea,.feed,.list,.hscroll,.lscroll,.msgs,.chatwrap,.frames,.top,.dtop,.hbar,.hero,.phero,.lhero,.chdr,.hfollow,.hchips,.stories,.sec,#mount'))return;
